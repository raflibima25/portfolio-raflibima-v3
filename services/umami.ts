...
export const getAllWebsiteData = async (): Promise<UmamiResponse> => {
  const { websites, is_active } = UMAMI_ACCOUNT;

  // Guard: jika fitur tidak aktif, return data kosong
  if (!is_active) {
    return {
      pageviews: [],
      sessions: [],
  ...
  const results = await Promise.all(
    websites.map(async (w) => {
      ...
      return {
        ...
        // Fallback ke struktur kosong jika API gagal / belum dikonfigurasi
        websiteStats: st.data?.pageviews
          ? st.data
          : {
              pageviews: { value: 0 },
              visitors: { value: 0 },
